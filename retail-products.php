...
$sql = "select product_id, product_name, product_description, product_price, product_image, product_inStock, product_status, product_update_date
from wdv341_products order by product_name ASC;";
...
<body>
	...
	<?php while($row=$result->fetch()){ ?>
        ...
        <div class="productBlock">
           ...
           <?php if (!empty($row['product_status'])){echo ' <p class="productStatus">'.$row['product_status'].'</p>  ';}?>          
           <?php if($row['product_inStock']<10){ echo '<p class="productLowInventory"> # In Stock: '.$row['product_inStock'].' </p> ';} else {echo ' <p class="productInventory">  # In Stock: '.$row['product_inStock'].' </p>';}; ?>
        </div>
